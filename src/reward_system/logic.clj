...
(defn treat-levels [levels vertice inviteds]
 (apply merge levels (doall (map 
                             #(hash-map % (inc (get levels vertice))) 
                             inviteds))))

(defn confirmed-inviteds-quantity [inviteds]
  (reduce + 
   (doall (map 
           #(if (utility/contains-set (read-string (name %)) @data/confirmed-inviteds) 1 0)
           inviteds))))

(defn bfs [graph levels visited adjacent]
  (lazy-seq
    (if (empty? adjacent)
      nil
      (let [vertice (peek adjacent)
            inviteds (graph vertice)
            confirmed-inviteds (confirmed-inviteds-quantity inviteds)
            level (get levels vertice)
            points (* (utility/pow 0.5 level) confirmed-inviteds)]
        (cons points (bfs
                       graph
